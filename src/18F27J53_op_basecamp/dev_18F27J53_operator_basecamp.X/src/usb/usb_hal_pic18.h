...
//------------------------------------------------------------------------------
    ...
    //STALLIE, IDLEIE, TRNIE, and URSTIE are all enabled by default and are required
        ...
        #define USBEnableInterrupts() {RCONbits.IPEN = 1;IPR2bits.USBIP = 0; \
            PIE2bits.USBIE = 1;INTCONbits.GIEL = 1;INTCONbits.GIEH = 1;}
        /*  GIEH enables high and low priority interrupts when IPEN is set.
         *  Low priority interrupts will not vector if IPEN and GIEL are set but GIEH
         *  is clear. -> set GIEH, even if low priority interrupts only! */
