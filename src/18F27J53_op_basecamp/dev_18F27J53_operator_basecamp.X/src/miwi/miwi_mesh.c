...
// MY_ADDRESS_LENGTH == 4 (always)
// and we're using the myMODEL and myUID identifiers
// as unique long address
//uint8_t myLongAddress[4] @ 0x01FFF4;
uint8_t myLongAddress[4];
                        ...
                        case MAC_COMMAND_BEACON_REQUEST:
                            {
                                ...
                                if(MiWiStateMachine.bits.memberOfNetwork)
                                {
                                    ...
                                    {
                                        // Patch nio - prevent beacon collision
                                        // no jitter for PAN_COORDINATOR
                                        if (role == ROLE_COORDINATOR)
                                        //#if !defined(TARGET_SMALL)
                                        {
                                            // static 20ms jitter
                                            // to let the PAN answer if
                                            // it's in vincinity
                                            uint8_t jitter = 20;
                                            MIWI_TICK t1, t2;
                                            while( jitter > 0 )
                                            ...
                                            // + random jitters
                                            // if another coordinator wants to
                                            // answer
                                            BroadcastJitter(20);
                                            BroadcastJitter(20);
                                            BroadcastJitter(20);
...
bool MiApp_ProtocolInit(bool bNetworkFreezer)
{
    ...
    #if defined(ENABLE_NVM) // lecture de l'adresse longue
