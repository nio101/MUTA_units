...
#include <stdint.h>
    ...
    const uint8_t SVALUE[]    = {CHANNEL1_SVALUE, CHANNEL2_SVALUE, CHANNEL3_SVALUE, CHANNEL4_SVALUE, CHANNEL5_SVALUE, CHANNEL6_SVALUE,
        ...
        void security_force_key_update()
        {
...
TX_END_HERE: 
    ...
    {                
        ...
        #if defined(ENABLE_ACK) && defined(ENABLE_RETRANSMISSION)  // THERE WAS A TYPO HERE on RETRANSMISSION!
...
/*
// stuff to modify the pulsing LED along with packet reception
extern update_LED();
typedef enum
{
    LED_OFF,
...
*/
void MIWIInterruptHandler(void)
{
    if( PHY_IRQ1 && PHY_IRQ1_En)    //&& IRQ1_INT_PIN==1 // Fait planter !?!
    {
        PHY_IRQ1 = 0;
        // debug output ON
        //LATCbits.LATC2 = PORTBbits.RB1;
...
RETURN_HERE:     
    ...
    // end of INT1 interrupt handler    
    
    // INT0 interrupt handler
    if(PHY_IRQ0 && PHY_IRQ0_En)
    {
        // show low level ouioui activity
        //status_led = LED_FULL;
        //update_LED();
